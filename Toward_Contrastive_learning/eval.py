...
class PretrainedModel(pl.LightningModule):
    def __init__(self, base_encoder, out_dim):
        super().__init__()
        self.model = ResNetSimCLR(base_model=base_encoder, out_dim=out_dim)
...
if __name__=="__main__":
    ...
    pretrained_model = PretrainedModel.load_from_checkpoint(checkpoint_path='/home/ssb/research/pretrain_r18_train/last.ckpt',model=extractor, strict=False)
    ...
    trainLoader = DataLoader(stl10_train, batch_size=batch_size, shuffle=True, num_workers=8, drop_last=True)
    valLoader = DataLoader(stl10_test, batch_size=1, shuffle=False, num_workers=4, drop_last=False)
    #model = Transfer(model=extractor, lr=0.0003, weight_decay=0.0008, len=len(trainLoader))
    model = Transfer(model=pretrained_model, lr=0.0003, weight_decay=0., len=len(trainLoader))
